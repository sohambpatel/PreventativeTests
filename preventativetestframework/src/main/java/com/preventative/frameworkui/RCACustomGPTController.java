...
public class RCACustomGPTController {
...
//    }
    ...
    @FXML
    void getRecommendationRCA(ActionEvent event) throws Exception {
        try {
            String propertyfilepath = getPATH();
            configProperties = new Properties();
            configProperties = PropertyFileLoader.readPropertyValues(propertyfilepath);
        ...
        if (System.getProperty("os.name").contains("Windows")) {
            stacktrace= SingletonFileHandler.getInstance().readFile(stacktraceRCA.getText());
        } else {
            stacktrace = SingletonFileHandler.getInstance().readFile(stacktraceRCA.getText());
        }
        String path;
        if (System.getProperty("os.name").contains("Windows")) {
            path = System.getProperty("user.home") + "\\Downloads\\RCA.info";
        } else {
            path = System.getProperty("user.home") + "/Downloads/RCA.info";
        }
        System.out.println("Prompt used for this request is: " + PromptHandler.promptReturner("RCAPROMPT"));
        SingletonFileHandler.getInstance().writeToFile(path, genAIHandler.generateRecommendation(PromptHandler.promptReturner("RCAPROMPT"), stacktrace));

        Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
        alert.setContentText("RCA task completed, \nCheck the logs for more details");
        alert.show();
    }
}