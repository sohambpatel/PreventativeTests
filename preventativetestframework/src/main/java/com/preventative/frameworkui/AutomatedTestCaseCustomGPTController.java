...
public class AutomatedTestCaseCustomGPTController {
    ...
    public static String automationfilecontent;
...
//    }
    ...
    @FXML
    void getRecommendationAutomatedTest(ActionEvent event) throws Exception {
        try {
            String propertyfilepath = getPATH();
            configProperties = new Properties();
            configProperties = PropertyFileLoader.readPropertyValues(propertyfilepath);
        ...
        if (System.getProperty("os.name").contains("Windows")) {
            automationfilecontent= SingletonFileHandler.getInstance().readFile(OBSERVEROUTPUT.getText());
        } else {
            automationfilecontent = SingletonFileHandler.getInstance().readFile(OBSERVEROUTPUT.getText());
        }
            String path;
        if (System.getProperty("os.name").contains("Windows")) {
            path = System.getProperty("user.home") + "\\Downloads\\automatedtestcases.testcase";
        } else {
            path = System.getProperty("user.home") + "/Downloads/automatedtestcases.testcase";
        }
        System.out.println("Prompt used for this request is: " + PromptHandler.promptReturner("AUTOMATEDTESTCASEPROMPT"));
        SingletonFileHandler.getInstance().writeToFile(path, genAIHandler.generateRecommendation(PromptHandler.promptReturner("AUTOMATEDTESTCASEPROMPT"), automationfilecontent));
